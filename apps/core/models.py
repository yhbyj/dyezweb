...
class Major(BaseModel):
    ...
    category = models.ForeignKey(MajorCategory,
                                 on_delete=models.CASCADE,
                                 verbose_name="专业类别")
    ...
    status = models.SmallIntegerField(default=1,
                                      choices=status_choices,
                                      verbose_name='专业状态')
    quota = models.IntegerField(default=35,
                                verbose_name='招生人数')
...
class Admission(BaseModel):
    ...
    # 户口性质，默认值为(0, "农")
    hukou = models.SmallIntegerField(
        default=0,
        choices=HUKOU_CHOICES,
        verbose_name='户口性质'
    )
    # 民族性质，默认值为(0, "汉")
    ...
    id_card_no = models.CharField(
        max_length=18,
        verbose_name='身份证号码',
        validators=[RegexValidator(r'\d{17}\w{1}',
    ...
    # 初中毕业学校，默认值(0, "安恬初中")
    graduate_school = models.SmallIntegerField(
        default=0,
        choices=GRADUATE_SCHOOL_CHOICES,
        verbose_name='初中毕业学校'
    )
    # 担任职务
    ...
    majors = models.ManyToManyField(
        Major,
        through='AdmissionMajor',
        through_fields=('admission', 'major'),
        verbose_name='专业'
    )
    ...
    confirmed = models.BooleanField(
        default=False,
        verbose_name='预报名确认状态'
    )
    # 确认文书
    confirmed_with = models.TextField(
        default='你已成功预报名，需网上报名时再次确认，优先录取。',
        max_length=200, verbose_name='确认文书'
    )
    # 确认人
...
class ThreeCompetitionRule(BaseModel):
    ...
    category = models.ForeignKey(
        ThreeCompetitionRuleCategory,
        on_delete=models.CASCADE,
        verbose_name="三项竞赛评分细则类别"
    )
    name = models.CharField(
        max_length=50,
        verbose_name='三项竞赛评分细则名称'
    )
    code = models.CharField(
        max_length=6,
        null=True,
        blank=True,
        verbose_name="三项竞赛评分细则代码"
    )
    desc = models.TextField(
        null=True,
        blank=True,
        verbose_name="三项竞赛评分细则描述"
    )
