...
class UserSerializer(serializers.ModelSerializer):
    ...
    def validate_code(self, code):
        ...
        sms_codes = SmsCode.objects.filter(mobile=self.initial_data['mobile']).order_by('-add_time')
        if sms_codes:
            # 获取最新的一条
            last_sms_code = sms_codes[0]
            # 验证码有效期为30分钟
            five_minutes_ago = timezone.now() - timezone.timedelta(days=0, minutes=30, seconds=0)
